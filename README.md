...
The shape is fixed by seven decisions, each written down in [`docs/adr/`](https://github.com/lacodda/austeris/tree/main/docs/adr):
...
- **An entry balances by construction** ([0007](https://github.com/lacodda/austeris/blob/main/docs/adr/0007-entries-balance-by-construction.md)). An entry is a header and its lines, and the lines sum to zero in every currency they touch — a deferred constraint trigger in PostgreSQL, not a check in the code that usually writes. One rule covers a purchase, a transfer, a split receipt and an exchange, and no balance is ever stored.
